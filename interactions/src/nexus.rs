...
impl UrlData {
    ...
    pub fn get_url(&self) -> String {
        ...
        format!(
            ...
            self.game_name, self.mod_id, self.file_id, self.key, self.expires
        )
    }
}
...
pub fn deconstruct_nxm<'life>(link: &'life str) -> UrlData {
    ...
    // extrapolate from query string
    {
        ...
        let temp_data = link.split('?').skip(1).collect::<Vec<&str>>()[0]
            .split('&')
            .collect::<Vec<&str>>();
...
pub async fn download_mod(api_key: &str, nxm_link: &str, path: impl AsRef<Path>) {
    ...
    let response = client
        ...
        .header(
            "apikey",
            HeaderValue::from_str(api_key).expect("Expected to be able to read header from str"),
        )
        .send()
        .await
    ...
    let text = response
        .text()
        .await
        .expect("Expected to be able to convert response to text");
    let json = serde_json::Value::from_str(&text.as_str())
        .expect("Expected to be able to parse json data");
    ...
    dest.write_all(&bytes)
        .expect("Expected to be able to write bytes to the file");
}
