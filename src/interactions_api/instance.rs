pub mod types {
    ...
    impl std::fmt::Display for GameNotInstalled {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            Ok(println!(
                "Error, could not find app with id {} in any known Steam libraries",
                self.appid
            ))
        }
    }
...
pub fn ensure_game_init(appid: i32) -> JobStatus {
    ...
    if !match std::fs::exists(format!("{game_path}/RusticModManager/instances.json")) {
        ...
        let mut instances_json =
            match std::fs::File::create(format!("{game_path}/RusticModManager/instances.json")) {
                Ok(v) => v,
                Err(e) => return JobStatus::FailedPermission(e),
            };
...
pub fn create_instance(appid: i32, instance_metadata: InstanceMetadata) -> JobStatus {
    ...
    match std::fs::create_dir_all(format!(
        "{game_path}/RusticModManager/instances/{}",
    ...
    // add instance to instances.json
    {
        // open the file
        let raw_json_data =
            match std::fs::read_to_string(format!("{game_path}/RusticModManager/instances.json")) {
                Ok(v) => v,
                Err(e) => return JobStatus::FailedPermission(e),
            };
        ...
        let mut array = match old_json.as_array() {
            Some(v) => v.clone(),
            None => {
                return JobStatus::FailedCast(String::from(
                    "Failed to cast instances.json root element to array",
                ));
            }
        };
        ...
        // write data to file
        match std::fs::write(
            format!("{game_path}/RusticModManager/instances.json"),
            new_raw_json_data,
        ) {
